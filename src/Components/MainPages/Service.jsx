      ...
      <div className="lg:w-10/12 w-11/12 h-auto mx-auto">
        <div className=" flex justify-evenly lg:flex-row flex-col gap-4 my-10">
          {services.map((service, index) => (
            <div
              key={index}
              className="w-full h-[350px] bg-white text-black border-[5px] flex justify-center space-y-10 items-center flex-col relative overflow-hidden"
              ref={(el) => (cardsRef.current[index] = el)}
            >
              <img src={service.svg} alt="" />
              <p className="text-[2rem] font-bold">{service.title}</p>
              <p className="px-10 text-center text-lg text-gray-500 font-semibold card-paragraph">
                {service.description}
              </p>
